...
public class OutgoingLogDAO extends GenericDAO implements BabbleOutgoingLogDAO {
    ...
    public boolean putOutgoingLog(OutgoingLog outgoingLog) {
        ...
        try (
        	Connection conn = dbCredentials.getConnection();
        		PreparedStatement pstmt = conn.prepareStatement("INSERT INTO OutgoingLog (Uuid,origin,destination,message,networkuuid,sender,messagestatusuuid) VALUES (?,?,?,?,?,?,?);");
        		)
        		{
    ...
    public List<OutgoingLog> getOutgoingLog(Account account, int fromIndex, int toIndex) {
        ...
        try (
            ...
            PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM outgoingLog where sender =  ?  ORDER BY logtime DESC  LIMIT ? OFFSET ?;");
        		)
        		{
